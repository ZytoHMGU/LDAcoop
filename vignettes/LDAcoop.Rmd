...
LDA_plot_activity(LDA_obj = out[[1]],uncertainty.band = T,xlim = c(0,500))
#statmod::elda(tested = BT20$`# Tested`,
#              dose = BT20$`S-value`,
#              response = BT20$`# Clonal growth`)
#          Lower Estimate    Upper
#Group 1 363.467 290.5343 232.2362
lines(x = c(0,600),y = c(0,-600/290.5343),lwd=2,col="blue")
polygon(x = c(0,600,600,1),y = c(0,-600/363.467,-600/232.2362,0),border = F,
...
100 cells is higher than the activity of 100 separated single cells. In terms
of single cell activity, 100 cells act as if they were more.
...
will be negative.
 - thus, in between those two numbers there will be cell numbers, where no-one 
...
positive wells. Following the single-hit Poisson model 
(SHPM, $P_{\lambda}(k=0) = e^{-p s}$), the expectation 
is:
\[\mu = 1-e^{-ps}.\]
...
Note that it is implicitely assumed, that those cells that are non-survivers 
